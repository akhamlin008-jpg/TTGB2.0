...
# ── downloadable trade detail CSV ──
st.subheader("Trade Detail — Downloadable CSV")
st.caption("One row per ticker per rebalance event. Import into Koyfin or any analysis tool.")

if not res["rebal_log_detail"].empty:
    detail = res["rebal_log_detail"].copy()
    # Format dates as strings for clean CSV
    ...
    st.download_button(
        label="Download Trade Detail CSV",
        data=csv_buf.getvalue(),
        file_name="quarterly_rebal_trade_detail.csv",
        mime="text/csv",
    )
else:
    st.info("No trade detail available.")
